<div id="layout-items">
    <div>
        <h1>تحلیل و طراحی سیستم‌ها</h1>
        <br><br>
        <a href="">میز کار</a>
        <img src="/assets/svg/main/long-arrow-alt-left.svg" alt="">
        <span>درس های من</span>
        <img src="/assets/svg/main/long-arrow-alt-left.svg" alt="">
        <a href="">تحلیل و طراحی سیستم‌ها (۱۳۱۲۰۴۶_۰۱)</a>
        <img src="/assets/svg/main/long-arrow-alt-left.svg" alt="">
        <a href=""> تالار اعلانات</a>
        <input style="float:left" class="search-input" type="text" name="" placeholder="جستجو در تالارهای گفتگو">
    </div>
    <div>
        <div>
            <img style="float: left; height: 1rem; vertical-align: middle; cursor: pointer" src="/assets/svg/main/cog.svg" alt=""><br>
            <h1>تالار اعلانات</h1>
            <P>اعلانات و خبرهای عمومی</P>
            <ul>
                <li>گفتگو</li>
                <li>شروع‌کنندهٔ گفتگو</li>
                <li>آخرین مطلب</li>
                <li>تعداد مطالب</li>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="/forum/discuss/51">آزمون پروژه عملی</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>14 دی 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>14 دی 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون پایانترم</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>6 دی 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>6 دی 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">کلاس آنلاین رفع اشکال - دوشنبه ۶ دی‌ماه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>2 دی 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>2 دی 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون فصل 9</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>12 آذر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>12 آذر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه گام های پنج، شش و هفت پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>7 آذر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>7 آذر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">کلاس آنلاین رفع اشکال - شنبه ۲۹ آبان‌ماه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>28 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>28 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون فصل 7</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>27 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>27 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">گام چهار پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>24 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>24 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون فصل 5</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>11 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>11 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">گام سه پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>8 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>8 آبان 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون فصل 4</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>28 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>28 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">گام دو پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>25 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>25 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه مربوط به تحویل های آنلاین در طول ترم</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>24 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>24 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">فرم میزان فعالیت اعضا</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>21 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>21 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون فصل 3</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>19 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>19 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">برگزاری کلاس آنلاین - ۱۷ مهرماه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>16 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>16 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون فصل 2</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>15 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>15 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">ویدیو های ارائه های 1 و 2</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>14 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>14 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">گام یک پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>10 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>10 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">ارائه انلاین - 02</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>9 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>9 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">کلاس آنلاین - ۱۰ مهرماه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>9 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>9 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">ارائه انلاین - 01</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">گام صفر پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">فایل نهایی اختصاص پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">انتخاب پروژه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>3 مهر 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه برگزاری آزمون فصل 1</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>30 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>30 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">اطلاعیه ساخت گروه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>29 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>29 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">بارگذاری فایل کلیات پروژه عملی و جدول زمان بندی</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>26 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f1.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>امیرحسین باباییان</div>
                                <div>26 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #ddd">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">بارگذاری فایل‌های فصل اول</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>26 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>26 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
            <ul style="border-bottom: 0.1rem solid #000">
                <div style="display: flex; align-items: center">
                    <li>
                        <img style="margin: 0.5rem; vertical-align: middle" src="/assets/svg/main/star.svg" alt="">
                        <a href="">جلسه اول - کلاس آنلاین دوشنبه ۲۲ شهریورماه</a>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>26 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div>
                            <div style="width: 20%; float: right">
                                <img style="border-radius: 1rem; height: 2rem" src="/assets/media/image/f2.jpg" alt="">
                            </div>
                            <div style="width: 80%; float: right">
                                <div>حسن بشیری</div>
                                <div>26 شهریور 1400</div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <span style="margin-right: 3rem">0</span>
                        <img style="float: left" src="/assets/svg/main/ellipsis-v.svg" alt="">
                    </li>
                </div>
            </ul>
        </div>
    </div>
</div>